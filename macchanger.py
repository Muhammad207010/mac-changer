...
def help_menu():
    ...
    arg.add_option("-m", "--macaddr", dest= "new_mac", help= "New Mac Address")
    arg.add_option("-s", "--show-mac", dest= "interface", help= "Mac Address")    
    arg.add_option("--anonymus-mode", dest= "interface", help= "generating random mac address")
    arg.add_option("--random-mac", dest= "interface", help= "generate a random mac address")
...
def main():
    try:
        if ( sys.argv[1] == '-i' or sys.argv[1] == '--interface' ) and ( sys.argv[3] == '-m' or sys.argv[3] == '--macaddr' ):
            ...
            exit()
        elif ( sys.argv[1] == '-i' or sys.argv[1] == '--interface' ) and ( sys.argv[3] == '-s' or sys.argv[1] == '--show-mac' ):
